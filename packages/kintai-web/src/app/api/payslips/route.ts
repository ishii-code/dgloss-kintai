/**
 * 給与明細の API ルート（App Router・サーバ専用）。
 *
 * - `GET /api/payslips?year=&month=` 対象年月の給与明細（総支給まで）を取得する（getPayslip）。
 *
 * employeeId は httpOnly cookie セッションから解決する。未ログインは 401。締め／従業員が
 * 無ければサービス層が not_found を返し 404 になる（画面側で「未発行」表示に用いる）。
 * 所得税・社会保険料は外部連携（未計上）で、明細では金額0のプレースホルダとして返る。
 */

import { getPayslip } from "@dgloss-kintai/api";
...
export async function GET(req: Request): Promise<Response> {
  ...
  const result = await getPayslip(query, {
  ...
  return Response.json({ payslip: result.value }, { status: OK_STATUS });
}
